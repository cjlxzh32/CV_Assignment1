...
def he(img):
    ...
    mapping = np.interp(cdf_input, cdf_target, xs)

    I_new = np.interp(I.flatten(), bins[:-1], mapping).reshape(I.shape)
    V_new = np.clip(I_new * 255, 0, 255).astype(np.uint8)
    hsv[:, :, 2] = V_new
